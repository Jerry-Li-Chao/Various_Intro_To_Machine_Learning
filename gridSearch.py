...
class GridSearch:
    ...
    def grid_search_configurations(self):
        ...
        # Using combinations and hp_names turn the list of value_combos
        configs_list = []
        "*** YOUR CODE HERE ***"
        # for each arrangement of learning_rate, batch_size and epochs
        # map their values to their names
        for one_arrangement in combinations:
            value_combos = {}
            index = 0
            for name in hp_names:
                value_combos[name] = one_arrangement[index]
                index += 1
            configs_list.append(value_combos)
    ...
    def perform_grid_search(self, train_dataset, eval_dataset):
        ...
        best_config = None
        "*** YOUR CODE HERE ***"
        GS_config = self.grid_search_configurations()
        for config in GS_config:
            model, score = self.train_and_eval(config, train_dataset, eval_dataset)
            # find the config with the best model and best score, simple as that
            if score > best_eval:
                best_eval = score
                best_model = model
                best_config = config
